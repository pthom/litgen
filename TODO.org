...
*** fail on  config.settings_file = "..." # const char * !!!!
  Trouver alternative / config


** auto-resize and other goodies from immvision

** on windows, link fails with msvc (but pip install works)

** Window CI, internal compiler error
 https://github.com/pthom/lg_imgui_bundle/actions/runs/3262823761/jobs/5360572719
  D:\a\lg_imgui_bundle\lg_imgui_bundle\bindings\pybind_imgui.cpp(1501): fatal error C1001: Internal compiler error.
...
cache tersed and qualified versions ?
replace deep copy by simple copy ?
  Avant
      time python demos/litgen/lg_imgui_bundle/autogenerate_imgui_bundle/autogenerate_imgui_bundle.py
      python   112.40s user 20.99s system 91% cpu 2:25.37 total
      Apres cache Qualified/ Terse: 90s
      Apres cache Known types: 50s
...
** Work on qualified types
   Fix enum in test_with_qualified_types()  (f5)

   Bad consequence / default namespace: see imgui_markdown.pyi:402

   Then, regenerate imgui & implot
