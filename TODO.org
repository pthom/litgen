...
Idée: LitgenWriterContext avec dedans Options + status (liste des modules, de leur stubs, etc.)
  En cours:
    creation pluggee a l'arrache dans code_to_adapted_unit :
    a revoir en faisant attention au points d'entree de l'API. On peut avoir plusieurs fichiers!

    Argh: _generate_code_impl_one_file bypasse les options de CppFileAndOptions
    ==> faire avec et virer les multiples options

    Argh2 : maintenant, probleme sur generation ImGui
