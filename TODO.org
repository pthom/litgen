...
* Litgen doc** Sources de stubs connues: $
...
Faire un tour dans cpp_to_python pour mettre en privé les function d'acces au comment qui ne font pas de replace


** Litgen Generator

BoxedTypes:
  ajout mode / amalgamation dans mylib
  recup unit test utiles
  delete BoxedTypes orig

  delete generate orig

  correct autogenerate imgui / bundle / template


#+begin_src python
  driver = LitgenDriver(options)

  driver.parse_header(filename)
  # or
  driver.parse_code(code, opt_filename)

  driver.write_generated_code()

#+end_src

**** Diagramme / Litgen driver
...
#+begin_src plantuml :file packages/litgen/doc/litgen_driver.png :eval yes
  ...
  LitgenContext *-- LitgenOptions

  class LitgenGenerator {
          + __init__(options)
          + process_cpp_header(filename)
          + process_cpp_headers([filenames])
          + process_cpp_code(code, filename: Opt[str])
          + write_generated_code(pydef_file, stub_file)
  }

  class LitgenGenerator_test_utils {
          pydef_code()
          stub_code()

  }

  LitgenDriver *-- LitgenContext

#+end_src

#+RESULTS:
[[file:packages/litgen/doc/litgen_driver.png]]


**** Diagramme / generation old




#+begin_src plantuml :file packages/litgen/doc/litgen_generate_diagram.png
  ...
  class GeneratedCode {
          pydef_code: CppPydefCode
          stub_code: PythonStubCode
          boxed_types_cpp_declaration: Opt[CppHeaderCode]
          ---

          + __init__(
          self,
          generated_one_files: [GeneratedCodeForOneFile],
          generated_boxed_type_code: Opt[GeneratedBoxedTypeCode],
...
#+RESULTS:
[[file:packages/litgen/doc/litgen_generate_diagram.png]]
...
Idée: LitgenWriterContext avec dedans Options + status (liste des modules, de leur stubs, etc.)
  En cours:


    Argh3 : maintenant, probleme sur generation ImGui
        Il faut virer generate_code_for_files, ajouter un merge
        simplifier a fond generate.py, ca fait mal a la tete...


L'idee d'utiliser une fake class risque d'echouer, car un meme namespace peut apparaitre plusieurs fois!
   Contournement possible: stocker tout, puis dump a la fin dans le meme fichier?
Idem dans pydef!
