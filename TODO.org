...
#+begin_src cpp
  ...
  struct Foo
  {
      ...
      auto  make_foo = [](int v[2]) -> std::unique_ptr<Foo> {
          return std::make_unique<Foo>(v);
      };


     auto lambda_make_foo = [](std::array<int, 2>) ->  -> std::unique_ptr<Foo> {
         // Call make foo
     };
...
** Wish List / Easy
   Add "overridable"/"pure" in stubs / trampoline
   Warning: Warning:
