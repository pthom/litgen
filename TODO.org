...
** test platforms
    remove symlink imgui_bundle (genant sous linux si install non editable)

    linux:
       si opencv apt: ok, mais immvision not avail bien que compilé
...
** Ajout https://github.com/andyborrell/imgui_tex_inspect

API / Load images & texture


** Guizmo

API add GizmoMatrices + tuple <bool, GizmoMatrices>
add API bool only?

Essayer d'utiliser au max glm

Checbox too dark => see style
slider instead of inputFloat?

Module ImGuizmo => gizmo

Rename ImGuizmoStl => ImGuizmo
