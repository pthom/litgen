...
*** X Impl:
    ...
    X SizeAuto
    X Save Geometry
        X Voir _ImguiAppParamsHelper (Rename to GeometryHelper) & AutoSizeAppWindow
    ...
    Doc HelloImGui:
      X PowerSave
      X AutoSize
      X API / Run simple autosize & powerSave
      N FAQ / full screen


*** X Windows / Scaling & Multimonitor

X Revoir LoadAdditionalFonts : doit pouvoir prendre en param un ContentScale (ou le lire dans runnerParams)
X Publier contentScale dans Params

*** Correct CI

*** Pub ImGuiBundle dans HelloImGui

*** Screenshot

*** Test notebook (& test exception)
...
check install on win & linux

pb zoom sous windows: trop petit
     float ratio = ImGui::GetIO().FontGlobalScale
     voir https://github.com/ocornut/imgui/issues/5081 et https://github.com/ocornut/imgui/blob/master/docs/FAQ.md#q-how-should-i-handle-dpi-in-my-application
