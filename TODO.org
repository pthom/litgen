...
** Add fiatlux to bindings: add generic way (cmake & co, group everything in one folder)
...
** Wheels
  pb / wheel arm64 macOS: essaie de charger Intel iPPIcv (because compiled on intel)?
  ==>


  Release v0.7.1 & release mac arm manually

  Readme


  Build Windows
    use OpenCV Fetch source?

  logger trop petit sous windows


** Fiatlux:
   Commit rename
   Bouger bindingd SuspendNodeEditorCanvas dans imgui_node


   ** F


FUCKING fix_brace_init_default_value:
   One should not restore the braces immediately, since adapters will need to be applied later
   and they will fail if braces are present....
     STOP STOP STOP => Back to holidays!!!!
