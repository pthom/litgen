...
*** Suffixe MY_API sur namespace ne sert qu'a faire ch...
    Plutot faire une liste de namespace exclus
...
** Wish List / Easy
  ...
  generator: error message with default pybind file template
  ...
  keep pointer and references layout inside CppType (int *x vs int *x vs int * x)

   add functions_api_prefixes via property to LitgenOptions?

   test trampoline with  test with py-mahi-gui

** test multiple litgen_glue_code with BoxedType: is there a link error / duplication?
    remove bindings/litgen_glue_code.h?
...
** static field: https://pybind11.readthedocs.io/en/stable/classes.html#instance-and-static-fields
test with
    struct A {
        static const int v_static = 1;
        static int f_static() { return 2; }
        int f() { return 3; }
    };


** Vectorizing: https://pybind11.readthedocs.io/en/stable/advanced/pycpp/numpy.html#vectorizing-functions
** Brace init:  https://pybind11.readthedocs.io/en/stable/advanced/classes.html#brace-initialization

** Add options on code, like return_reference:
*** Dynamic attribute (add py::dynamic_attr() at class construction)  https://pybind11.readthedocs.io/en/stable/classes.html#dynamic-attributes
*** https://pybind11.readthedocs.io/en/stable/advanced/functions.html#additional-call-policies
** Final classes: https://pybind11.readthedocs.io/en/stable/advanced/classes.html#binding-final-classes

** args/kwargs: https://pybind11.readthedocs.io/en/stable/advanced/functions.html#accepting-args-and-kwargs
