...
options litgen and srcml:
  replace regex lists by composed unique regex?
  make options names and doc consistent
  review srcmlcpp options: pas de filtre / header par default, etc (le faire dans litgen)
...
Add options:
    ...
    # class_exclude_by_name__regex: List[str] = []
