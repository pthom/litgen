...
** Styling
  ...
  see https://gist.github.com/dougbinks/8089b4bbaccaaf6fa204236978d165a9
  https://github.com/GraphicsProgramming/dear-imgui-styles

  Review styling / checboxes and markdown

** Add immvision

issue windows arm:
  --profile ..\external\immvision\conan_profile_win_arm.txt


** Add glfw bindings:
    Publish glfw and imgui_backends

    Need to copy dylib to bindings/imgui_bundle
      (use code from lg_cmake_utils)  (works with manual copy for now)
    Pb compat dylib glfw original vs glfw imgui_bundle?
    Test usage dans demo_imgui_color_text_edit()

** Renaming ImGui[A-Z]
    Review options:
      code_replacements / names_replacements / comments_replacements
      => need: class_names_replacements, functions_names_replacements, var_names_replacements, types_replacements, value_replacements (qui contient types_replacements)


** Add ImGui callbacks via patch std::function?

** Add ImGui integrations
