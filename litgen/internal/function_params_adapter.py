...
def _make_adapted_lambda_code_end(
    ...
    lambda_code = code_utils.replace_in_string_remove_line_if_none(lambda_code, {
        "maybe_return_r": maybe_return_r,
        "maybe_lambda_output_code": maybe_lambda_output_code,
    })
    lambda_code = code_utils.replace_in_string(lambda_code, {
        "_i_": _i_,
...
def _make_adapted_lambda_code(
    ...
    if function_adapted_params.lambda_to_call is not None:
        _lambda_captures_list.append("&" + function_adapted_params.lambda_to_call)
    elif is_method:
    ...
    lambda_template_end = code_utils.indent_code(
        lambda_template_end, indent_str=options.indent_cpp_spaces(), skip_first_line=True)
    ...
    lambda_code = lambda_template_code
    lambda_code = code_utils.replace_in_string_remove_line_if_none(lambda_code, {
        "maybe_lambda_input_code": maybe_lambda_input_code
    })
    lambda_code = code_utils.replace_in_string(lambda_code, {
        "_i_": _i_,
