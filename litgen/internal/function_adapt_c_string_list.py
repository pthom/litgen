...
def adapt_c_string_list(
    ...
    def needs_adapt():
        ...
        for param_0, param_1 in code_utils.overlapping_pairs(old_function_params):
            if (
                param_0.decl.is_c_string_list_ptr()
                and cpp_to_python.looks_like_size_param(param_1, options)
            ):
                return True
        return False
    ...
    def is_c_string_list(i: int):
        ...
        r = param.decl.is_c_string_list_ptr() and cpp_to_python.looks_like_size_param(
            param_next, options
        )
        return r
    ...
    def is_c_string_list_size(i: int):
        ...
        r = (
            param_previous.decl.is_c_string_list_ptr()
            and cpp_to_python.looks_like_size_param(param, options)
        )
        return r
    ...
    for i, old_param in enumerate(old_function_params):
        ...
        if is_c_string_list(i):
            ...
            lambda_adapter.lambda_input_code += (
                f"std::vector<const char *> {vec_name};\n"
            )
            lambda_adapter.lambda_input_code += f"for (const auto& v: {param_name})\n"
            lambda_adapter.lambda_input_code += (
                f"{_i_}{vec_name}.push_back(v.c_str());\n"
            )
            lambda_adapter.lambda_input_code += (
                f"{size_type} {size_name} = {casted_size_str};\n"
            )
    ...
    lambda_adapter.lambda_name = (
        function_adapted_params.function_infos.name + "_adapt_c_string_list"
    )
