...
def test_parse_function_decl():
    ...
    """
    code_utils.assert_are_codes_equal(code_to_fn_decl(code), "std::vector<std::pair<size_t, int>> enumerate(const std::vector<int> & xs);")
...
def test_parse_function():
    ...
    int foo() { return 42; }
    """
    code_utils.assert_are_codes_equal(code_to_fn_decl(code), """
        // Apply foo
        int foo()
        <unprocessed_block/>
    """);
