...
class AmalgamationOptions:
    ...
    # Controls which include styles are treated as local (i.e. inlined into the
    # amalgamation). When True (the default), only quoted includes
    # (`#include "..."`) are local and angle-bracket includes (`#include <...>`)
    # are always external, following the C/C++ convention. This is also required
    # when local_includes_startwith is empty (e.g. Dear ImGui, whose headers are
    # included as `#include "imgui.h"` with no prefix).
    # Set to False to additionally treat prefix-matching angle-bracket includes
    # (`#include <prefix/...>`) as local. local_includes_startwith must then be
    # non-empty, otherwise system headers like <vector> would be inlined.
    only_quoted_includes_are_local: bool
    ...
    def __init__(self) -> None:
        ...
        self.only_quoted_includes_are_local = True
...
def amalgamation_content(options: AmalgamationOptions) -> str:
    if len(options.local_includes_startwith) == 0 and not options.only_quoted_includes_are_local:
        raise ValueError(
            "AmalgamationOptions.local_includes_startwith is empty: "
            "set only_quoted_includes_are_local=True, otherwise every system include "
            "(e.g. #include <vector>) would be treated as local and litgen would try to inline it."
        )
...
def _is_local_include_line(options: AmalgamationOptions, code_line: str) -> bool:
    ...
    if _code_line.startswith(f'#include "{options.local_includes_startwith}'):
        return True
    if not options.only_quoted_includes_are_local and _code_line.startswith(
        f"#include <{options.local_includes_startwith}"
    ):
        return True
    return False
...
def _extract_local_include_file(code_line: str) -> str:
    ...
    (handles both `#include "file"` and `#include <file>`)
    """
    _code_line = code_line.strip()

    result = _code_line.replace('#include "', "").replace("#include <", "").replace('"', "").replace(">", "")
