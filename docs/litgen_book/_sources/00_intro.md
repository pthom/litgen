...
srcmlcpp and its associated tools, such as litgen, are under the [srcmlcpp and litgen License](https://github.com/pthom/srcmlcpp/LICENSE.txt).
...
```{note}
litgen uses [srcML](http://www.srcml.org) which is governed by the GNU GENERAL PUBLIC LICENSE Version 3 (see [COPYING.txt](https://github.com/srcML/srcML/blob/master/COPYING.txt)). It does not link to it, but calls it as an external tool.
```
